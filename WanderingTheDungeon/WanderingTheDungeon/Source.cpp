...
void levelUp(Object& player)
{
	...
	*/
	...
	std::normal_distribution<double> randomItemBonus(player.level, player.level / 3);

	//5. create new item with random values.
	Item loot{ (Item::Type)randomItemType(engine), (int)randomItemBonus(engine) };
	...
	*/
	if (player.inventory.find(loot.clasification) == player.inventory.end())
	{
		player.inventory.insert({ loot.clasification, loot });
	}
	else if(player.inventory.at(loot.clasification).bonusValue < loot.bonusValue )
	{
		player.inventory.insert({ loot.clasification, loot });
	}
}
...
int calculateAC(const Object& object)
{
	...
	{
		AC += object.inventory.at(Item::Type::armor).bonusValue;
	}

	if ( auto shield{ object.inventory.find(Item::Type::shield) };
		shield != object.inventory.end() )
	{
		AC += object.inventory.at(Item::Type::shield).bonusValue;
	}
...
int attack(const Object& object)
{
	...
	{
		potentialDamage += object.inventory.at(Item::Type::sword).bonusValue;
	}
