...
void displayBattle(const Object& player, const std::vector<Object>& monsters)
{
	...
	auto itemPrint = [](Item& item) -> void
		{
			std::cout << "  ";
			printItem(item);
			std::cout << std::endl;
		};
	std::for_each(player.inventory.begin(), player.inventory.end(), itemPrint);

	std::cout << std::endl << "  Monsters: " << std::endl;
	//use for_each

	int count{ 0 };
	auto monsterPrint = [&count](Object& monster) -> void
		{
			std::cout << "   " << count + 1 << ". ";
			printName(monster);
			std::cout << " h:" << monster.health << std::endl;
		};
	std::for_each(monsters.begin(), monsters.begin(), monsterPrint);
}
std::vector<Object> createMonsters(const Object& player)
{
	//use std::generate
	...
	auto lambda = [&](const Object& player) -> Object {
	...
	std::generate(monsters.begin(), monsters.end(), lambda);
	return monsters;
}
void monsterAttack(Object& player, const std::vector<Object>& monsters)
{
	...
	auto lambda = [](Object& monster, Object& player, bool strike) -> void
		{
			if (strike)
			{
				printName(monster);
	...
	std::for_each(monsters.begin(), monsters.end(), lambda);
...
void bringOutYourDead(std::vector<Object>& monsters)
{
	...
	monsters.erase(std::remove_if(monsters.begin(), monsters.end(), dead));
}